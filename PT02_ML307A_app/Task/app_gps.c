...
static int8_t calculateTheGPSCornerPoint(void)
{
    ...
    if (coursechange >= 15 && coursechange <= 45)
    {
        ...
        jt808SendToServer(JT808_LINK, TERMINAL_POSITION, &gpsfifo->gpsinfohistory[positionidnex[1]]);
        jt808SendToServer(JT808_LINK, TERMINAL_POSITION, &gpsfifo->gpsinfohistory[positionidnex[3]]);
    }
    else if (coursechange > 45)
    {
        ...
        jt808SendToServer(JT808_LINK, TERMINAL_POSITION, &gpsfifo->gpsinfohistory[positionidnex[0]]);
        jt808SendToServer(JT808_LINK, TERMINAL_POSITION, &gpsfifo->gpsinfohistory[positionidnex[1]]);
        jt808SendToServer(JT808_LINK, TERMINAL_POSITION, &gpsfifo->gpsinfohistory[positionidnex[2]]);
        jt808SendToServer(JT808_LINK, TERMINAL_POSITION, &gpsfifo->gpsinfohistory[positionidnex[3]]);
        jt808SendToServer(JT808_LINK, TERMINAL_POSITION, &gpsfifo->gpsinfohistory[positionidnex[4]]);
...
void gpsMileRecord(void)
{
	...
	else 
	{
		...
		{
			tick = 0;
			mile = calculateTheDistanceBetweenTwoPonits(gpsinfo->latitude, gpsinfo->longtitude, lastmilegpsinfo.latitude,\
            ...
            LogPrintf(DEBUG_ALL, "gpsMileRecord==>mile:%.3lf, totalmile:%.3lf, tickgap:%d", 
            mile, sysparam.mileage * (sysparam.milecal / 100.0 + 1.0), (sysinfo.sysTick - lastmilegpsinfo.tick));
