...
static uint8_t ble2instructionid[4];
...
void gpsRestoreUpload(void)
{
    ...
    if (sysparam.protocol == JT808_PROTOCOL_TYPE)
    {
        jt808TcpSend(JT808_LINK, (uint8_t *)dest, destlen);
    }
    else
    {
...
static void protoclparase80(uint8_t link, char *protocol, int size)
{
    ...
    else if (link == BLE1_LINK)
    {
        ...
        connhandle = bleDevGetHandleBySocketid(BLE1_LINK);
        if (connhandle >= 0)
        {
			getBle1Insid();
        	bleCentralSend(connhandle, instruction, instructionlen);
        }
		LogPrintf(DEBUG_ALL, "protoclparase80==>link:%d conhandle:%d", link, connhandle);

    }
    else if (link == BLE2_LINK)
    {
    	connhandle = bleDevGetHandleBySocketid(BLE2_LINK);
    	if (connhandle >= 0)
    	{
			getBle2Insid();
	        bleCentralSend(connhandle, instruction, instructionlen);
...
{
    ble2instructionid[0] = instructionid[0];
    ble2instructionid[1] = instructionid[1];
    ble2instructionid[2] = instructionid[2];
    ble2instructionid[3] = instructionid[3];
}
...
{
    instructionid[0] = ble2instructionid[0];
    instructionid[1] = ble2instructionid[1];
    instructionid[2] = ble2instructionid[2];
    instructionid[3] = ble2instructionid[3];
}
