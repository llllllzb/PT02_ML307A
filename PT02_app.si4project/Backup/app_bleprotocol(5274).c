...
static void devLoginInfoParser(char *sn, uint8_t connSuccess, deviceConnInfo_s *bleinfo)
{
	/* 已存在且正在连接 */
...
//        bleRelayList[ind].bleInfo.bleLost = 0;
        switch (data[readInd + 3])
        {
        	case CMD_DEV_LOGIN_INFO:
				devLoginInfoParser(data + readInd + 4, data[readInd + 19], bleInfo);
...
void bleProtocolSendEventTask(void)
{
	...
	if (devinfo->discState == BLE_DISC_STATE_COMP && devinfo->dataEvent != 0)
	{
		...
		if (devinfo->dataEvent & BLE_SEND_MASTERINFO_EVENT)
		{
			...
			bleSendProtocol(devinfo->connHandle, devinfo->charHandle, CMD_DEV_HEARTBEAT, param, 1);
