...
static bleSocketInfo_s	ble1ServConn, ble2ServConn, blePetServConn[DEVICE_MAX_CONNECT_COUNT];
...
static void ble2ServerInit(void)
{
	memset(&ble2ServConn, 0, sizeof(bleSocketInfo_s));
}

static void ble2InfoRegister(uint8_t *sn, float vol, uint8_t bat, uint16_t step)
{
	memcpy(ble2ServConn.loginSn, sn, 15);
	ble2ServConn.loginSn[15] = 0;
	ble2ServConn.vol  = vol;
	ble2ServConn.step = step;
	LogPrintf(DEBUG_ALL, "Ble[2]InfoRegister:SN[%s], vol:%f, bat:%d%%, step:%d", ble2ServConn.loginSn, ble2ServConn.vol, ble2ServConn.batlevel, ble2ServConn.step);
}

static void ble2ChangeFsm(NetWorkFsmState fsm)
{
	ble2ServConn.fsmstate = fsm;
}

void ble2ServerAdd(devSocketData_s *sockdata)
{
	ble2ServConn.use = 1;
	ble2InfoRegister(sockdata->SN, sockdata->vol, sockdata->bat, sockdata->step);
}

void ble2ServerLoginUpdate(uint8_t *sn)
{
	ble2ServConn.use = 1;
	memcpy(ble2ServConn.loginSn, sn, 15);
	ble2ServConn.loginSn[15] = 0;
	LogPrintf(DEBUG_ALL, "Ble2 server update sn:%s", ble2ServConn.loginSn);
}
...
void ble2ServerConnTask(void)
{
	...
	{
		ble2ChangeFsm(SERV_LOGIN);
		ble2ServConn.loginCount = 0;
		socketAdd(BLE2_LINK, sysparam.bleServer, sysparam.bleServerPort, bleServerSocketRecv);
		ble2InfoRegister(devInfo->sockData.SN, devInfo->sockData.vol, devInfo->sockData.bat, devInfo->sockData.step);
		return;
	}
	/* 如果蓝牙链路断开后连接别的SN号则重新登陆 */
	if (strncmp(devInfo->sockData.SN, ble2ServConn.loginSn, 15) != 0)
	{
		socketDel(BLE2_LINK);
	...
	{
		case SERV_LOGIN:
		    protocolSnRegister(ble2ServConn.loginSn);
            protocolSend(BLE2_LINK, PROTOCOL_01, NULL);
			break;
		case SERV_LOGIN_WAIT:
		...
		case SERV_READY:
            if (ble2ServConn.heartbeattick % sysparam.heartbeatgap == 0)
            {
                ...
                protocolInfoResiter(ble2ServConn.batlevel, sysinfo.outsidevoltage > 5.0 ? sysinfo.outsidevoltage : ble2ServConn.vol,
                                    dynamicParam.startUpCnt, dynamicParam.runTime);
                protocolSend(BLE2_LINK, PROTOCOL_13, NULL);
                //ble2InfoRegister(devInfo->sockData.SN, devInfo->sockData.vol, devInfo->sockData.bat, devInfo->sockData.step);
            }
...
int8_t blePetServerAdd(char *Sn)
{
	...
	{
		LogPrintf(DEBUG_BLE, "blePetServerAdd==>Fail,Sn[%s]", Sn);
		return -1;
	}
	int8_t i; 
	for (i = 0; i < DEVICE_MAX_CONNECT_COUNT; i++)
	{
		if (blePetServConn[i].use == 0)
		{
			LogPrintf(DEBUG_BLE, "blePetServerAdd==>Ok,Sn[%s],sockid:%d", Sn, BleSockId(i));
			return BleSockId(i);
		}
	}
	return -1;
...
int8_t blePetServerUploadUpdate(devSocketData_s *data)
{
	...
	{
		LogPrintf(DEBUG_BLE, "blePetServerUploadUpdate==>Fail,Sn[%s]", data->SN);
		return -1;
	}
	...
	LogPrintf(DEBUG_ALL, "Ble(%d)InfoRegister:vol:%f, bat:%d%%, step:%d", 
...
static void blePetChangeFsm(uint8_t index, NetWorkFsmState fsm)
{
	...
	LogPrintf(DEBUG_ALL, "blePetChangeFsm(%d)==>%d", index, blePetServConn[index].fsmstate);
}
