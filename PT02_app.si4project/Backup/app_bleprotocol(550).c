...
static void devLoginInfoParser(uint8_t ind, char *sn, uint8_t connSuccess, char *masterSn, deviceConnInfo_s *bleinfo)
{
	...
	{
		memcpy(bleinfo->sockData.SN, sn, 15);
		bleinfo->sockData.SN[15] = 0;
		bleinfo->sockSuccess = 1;
		bleinfo->socketId = blePetSearchServerSn(sn);
	...
	{
		/* 先判断主机sn号是否是自己 */
		if (strncmp(masterSn, dynamicParam.SN, 15) == 0)
		{
			memcpy(bleinfo->sockData.SN, sn, 15);
			bleinfo->sockData.SN[15] = 0;
			bleinfo->sockSuccess = 1;
			...
			bleSendDataReqSet(ind, BLE_SEND_MASTERINFO_EVENT);
			LogPrintf(DEBUG_BLE, "$$ Ble get login info==>Local link, Master[%s] Sn:[%s]", dynamicParam.SN, bleinfo->sockData.SN);
		}
		/* 进入该状态表示设备已通过别的PT02连接 */
		else
		{
			LogPrintf(DEBUG_BLE, "$$ Ble get login info==>Other link, Sn:[%s]", bleinfo->sockData.SN);
		}
		return;
	}
        ...
        switch (data[readInd + 3])
        {
        	case CMD_DEV_LOGIN_INFO:
 			...
 			case CMD_DEV_MASTER_INFO:
				LogPrintf(DEBUG_BLE, "$$ Ble master info respon");
...
void bleProtocolSendEventTask(void)
{
	...
	if (devinfo->discState == BLE_DISC_STATE_COMP && devinfo->dataEvent != 0)
	{
		...
		if (devinfo->dataEvent & BLE_SEND_MASTERINFO_EVENT)
		{
			...
			bleSendProtocol(devinfo->connHandle, devinfo->charHandle, CMD_DEV_HEARTBEAT, param, strlen(param));
