        ...
        for (i = 0; i < pEvent->dataLen; i++)
        {
            ...
            switch (cmd)
            {
                ...
                case GAP_ADTYPE_LOCAL_NAME_COMPLETE:
                    ...
                    {
                    	LogPrintf(DEBUG_BLE, "PT13_BroadName:[%s]", scaninfo.broadcaseName);
...
void bleDisconnDetect(void)
{
	...
	for (i = 0; i < DEVICE_MAX_CONNECT_COUNT; i++)
	{
		if (devInfoList[i].use)
		{
			LogPrintf(DEBUG_ALL, "ble update tick:%d", devInfoList.updateTick);
			if (sysinfo.sysTick - devInfoList[i].updateTick >= 180)
			{
				LogPrintf(DEBUG_ALL, "ble lost tick:%d ,systick:%d", devInfoList.updateTick, sysinfo.sysTick);
				bleDevConnDel(i);
			}
		}
	}
